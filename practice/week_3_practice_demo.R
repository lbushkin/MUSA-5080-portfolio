...
ggplot(county_data)+
  aes(x=estimate, y = moe_pct, color = "skyblue")+
  geom_point()
...
# everything wrong with the graph: x-axis jumbled/overcrowded, y-axis is sci notation, columns out of order, bars not visible
# let's make it less horrible
# reduce to 15 most unreliable (for clarity's sake)

county_data %>%
  arrange(desc(moe_pct)) %>%
  slice_head(n = 15) %>%   #to get top 15 highest moe's
  ggplot(aes(x = NAME, y = estimate)) +
  geom_col() +
  geom_errorbar(aes(ymin = estimate - moe, ymax = estimate + moe))

#slightly better, but x-axis still illegible
...
county_data %>%
  arrange(desc(moe_pct)) %>%
  slice_head(n = 15) %>%
  ggplot(aes(x = reorder(NAME, estimate), y = estimate)) +    #reorder to sort names by size of estimate
  geom_col() +
  geom_errorbar(aes(ymin = estimate - moe, ymax = estimate + moe)) +
  coord_flip()
...
county_data %>%
  arrange(desc(moe_pct)) %>%
  mutate(NAME = str_remove(NAME, " County, New York")) %>%    #str_remove to cut out county,NY in name column
  slice_head(n = 15) %>%
...
county_data %>%
  ...
  ggplot(aes(x = reorder(NAME, estimate), y = estimate)) +
  geom_col() +
  geom_errorbar(aes(ymin = estimate - moe, ymax = estimate + moe), width = 0.3) +  #width to make the error bars smaller/larger
  coord_flip()
...
county_data %>%
  ...
  coord_flip() +
  theme_minimal() +
  scale_y_continuous(labels = scales::comma) +     #signal that y-axis is continuous scale, and numbers have commas
  labs(
...
# you want the total number of people below poverty across several counties. You need the MOE for that combined number.
# you don't just add the MOEs together!
...
region_total      # 9,510
region_total_moe  # 884, not 141 + 493 + 720 = 1,354 (simple addition overstates the moe)
...
get_poverty <- function(yr) {           #this function is to get poverty stats for whatever year specified
  get_acs(
    geography = "county",
    ...
    mutate(period = paste0(yr - 4, "-", yr))   #to add a column that adds the ACS time period (i.e. 2019-2023)
}

# Reformat to wide to put the years side by side

poverty_change <- bind_rows(   #to combine results of 2018 and 2023 in one table
  get_poverty(2018),
  get_poverty(2023)
)

poverty_wide <- poverty_change %>%                  #wide so that each county is one observation/row
...
poverty_tested <- poverty_wide %>%
  mutate(
    ...
    z        = change / se_diff,
    significant = abs(z) > 1.645   #creates True/False column for whether or not change is significant
  )
...
poverty_tested %>%
  ...
  geom_col(aes(fill = change > 0)) +   #different colors for whether change was positive or negative; created dumb legend tho
...
## Create a fancy reliability table like in the paper/reading:
...
reliability_table <- county_data %>%
  mutate(
    ...
    dot = case_when(
      cv < 12  ~ "🟢",   #these symbols come from holding down windows key and then pressing period
      cv <= 40 ~ "🟡",
      TRUE     ~ "🔴"
    ),
    flag = case_when(
